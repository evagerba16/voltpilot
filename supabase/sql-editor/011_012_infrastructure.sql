-- VoltPilot infrastructure: migrations 011 + 012
-- Run in Supabase SQL Editor: https://supabase.com/dashboard/project/njgouumdrmwezmjzheug/sql/new
-- Requires 001-010 (team management + can_edit_estimates / can_view_org_data).
...
drop policy if exists "Team can view estimate AI sessions" on public.estimate_ai_sessions;
drop policy if exists "Editors can manage estimate AI sessions" on public.estimate_ai_sessions;
drop policy if exists "Editors can update estimate AI sessions" on public.estimate_ai_sessions;
drop policy if exists "Team can view estimate AI messages" on public.estimate_ai_messages;
drop policy if exists "Editors can insert estimate AI messages" on public.estimate_ai_messages;
...
drop policy if exists "Team can view proposal revisions" on public.proposal_revisions;
drop policy if exists "Editors can insert proposal revisions" on public.proposal_revisions;
drop policy if exists "Team can view proposal status history" on public.proposal_status_history;
drop policy if exists "Editors can insert proposal status history" on public.proposal_status_history;
drop policy if exists "Team can view proposal emails" on public.proposal_emails;
drop policy if exists "Editors can insert proposal emails" on public.proposal_emails;
drop policy if exists "Team can view proposal views" on public.proposal_views;
drop policy if exists "Team can view proposal comments" on public.proposal_comments;
drop policy if exists "Editors can insert internal proposal comments" on public.proposal_comments;
...
-- Record applied migrations (for npm run db:verify / apply-migrations tracking)
create table if not exists public.schema_migrations (
  filename text primary key,
  applied_at timestamptz not null default now()
);
insert into public.schema_migrations (filename) values ('011_ai_estimate_assistant.sql') on conflict (filename) do nothing;
insert into public.schema_migrations (filename) values ('012_proposals_workflow.sql') on conflict (filename) do nothing;
